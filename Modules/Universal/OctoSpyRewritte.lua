	...
	task.spawn(function() -- Instance1
		...
		local fireServerHook, fireServer2Hook, invokeServerHook, fireHook, invokeHook = (newc(function(...)
			...
			if not ignore[self] and not ignore[self.Name] then
				...
				table.insert(insertionQueue, {log, "To", "local args = "..args.."\n\n"..tostr(self)..":FireServer(unpack(args))", nil, self, args, "FireServer", CallerScript = getcallingscript()})
			end
			...
			if not ignore[self] and not ignore[self.Name] then
				...
				local t = {log, "To", "local args = "..args.."\n\n"..tostr(self)..":FireServer(unpack(args)) -- Unreliable FireServer", nil, self, args, "FireServer", CallerScript = getcallingscript()}
				table.insert(insertionQueue, t)
			end
			...
			if not ignore[self] and not ignore[self.Name] then
				...
				local t = {log, "To", "local args = "..args.."\n\n"..tostr(self)..":InvokeServer(unpack(args))", nil, self, args, "InvokeServer", CallerScript = getcallingscript()}
				table.insert(insertionQueue, t)
		...
		cons[#cons+1] = addButton("Run code").MouseButton1Click:Connect(function()
			...
			local s,e = loadstring(code.Text)
			if s then
				s()
        ...
        cons[#cons+1] = addButton("Get calling script").MouseButton1Click:Connect(function()
			if not selected then
				return notification("Please, select a remote first!")
			end

			if selected.CallerScript then
				code.Text = tostr(selected.CallerScript)
			else
				notification("No caller script found!")
			end
		end)
